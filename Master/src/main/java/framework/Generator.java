        ...
        for (int i = 1; i <= amount; i++) {
            ...
            if (returnType.equals("char")) {
                if (subConfig.containsKey("correlation")) {
                    ...
                    Utils.multCharArrayToFile(arrays, storagefolder + file, subConfig.getBoolean("withIndex"));
                } else {
                    Utils.storeIntAsChar(generatedValues, storagefolder + file, subConfig.getBoolean("withIndex"));
                }
                continue;
            }
            ...
            if (returnType.equals("String")) {
                if (!(subConfig.containsKey("correlation"))){
                    ...
                    Utils.multStringArrayToFile(stringArrays,storagefolder+file,subConfig.getBoolean("withIndex"));
                }
    ...
    public ArrayList<char[]> generateCorrelationChar(int[] inputArray, Configuration config) {
        ...
        if (correlationType.equals("functional dependent")) {
            ...
            for (int i = 0; i < inputArray.length; i++) {
                ...
                correlatedArrayChar[i] = (char) (((correlatedArray[i] - firstLetter) % numberOfChars) + firstLetter);
            }
        } else {
